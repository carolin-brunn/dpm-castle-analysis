...
def main():
    ...
    for year in l_year: 
        ...
        for x in range(1,13):
            ...
            output_file = "transformed_date_" +  year + "-" + month + "-eletricity_consumption.csv"
            new_df.to_csv((path + output_file), sep = ",", index = False)
        
    # 2015 has only 1.1. at midnight (0 o'clock)
    year = "2015"
    month = "01"
    input_file = year + month + "eletricity_consumption.csv"
    df = pd.read_csv((path + input_file), sep = ",")
