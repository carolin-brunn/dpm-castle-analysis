...
class Cluster():
    ...
    def insert(self, element: Item):
        ...
        """NEW"""
        self.diverse_pids.add(element.data["pid"])
    ...
    def remove(self, element: Item):
        ...
        """NEW: add diverse pid handling"""
        if not element.data["pid"] in [e.data["pid"] for e in self.contents]:
            self.diverse_pids.remove(element.data["pid"])
    ...
    def distance(self, other: Item) -> float:
        ...
        for header, header_range in self.ranges.items():
            ...
            other_item = other[header] # NEW
            ### NEW ### distance of dates:
            # idea: return max of day distance to lower or upper range limit
            if(isinstance(other_item, dt.date)):
                dif  = max((header_range.upper - other_item), (header_range.lower - other_item))
                total_distance += abs(dif.days)
            
            elif(isinstance(other_item, dt.time)):
                low_delta = dt.timedelta(hours = header_range.lower.hour, minutes = header_range.lower.minute, seconds = header_range.lower.second)
                up_delta = dt.timedelta(hours = header_range.upper.hour, minutes = header_range.upper.minute, seconds = header_range.upper.second)
                item_delta = dt.timedelta(hours = other_item.hour, minutes = other_item.minute, seconds = other_item.second)
                
                dif  = max((up_delta - item_delta), (low_delta - item_delta))
                total_distance += abs(dif.seconds)
                    
            ### treating integer ranges
            else:
                dif =  other_item - (header_range.difference())
